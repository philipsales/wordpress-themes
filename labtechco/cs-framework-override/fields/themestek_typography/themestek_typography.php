 ...
 *
 */
class CSFramework_Option_themestek_typography extends CSFramework_Options {
  ...
  public function output() {
    ...
    $google_json   = ts_cs_get_google_fonts();
    ...
    if( is_object( $google_json ) ) {
	  ...
	  echo '<div class="cs-ts-typography-row1">';
	  
	  
	  /**
	   *  Font Family
	   */
	  echo '<div class="ts-option-col-6 cs-ts-typography-dropdown-w cs-ts-typography-family-w">';
	  echo '<label class="ts-typography-family">';
	  echo '<small>'.esc_attr__( 'Font Family', 'labtechco' ).'</small><br>';
      ...
      echo '<optgroup label="'. esc_attr__( 'Web Safe Fonts', 'labtechco' ) .'">';
      ...
      echo '</optgroup>';
      echo '<optgroup label="'. esc_attr__( 'Google Fonts', 'labtechco' ) .'">';
	   ...
	   */
		if( ! empty( $is_backup_family ) ) {
			echo '<div class="ts-option-col-6 cs-ts-typography-dropdown-w cs-ts-typography-bfont-family-w">';
			echo '<label class="ts-cs-backup-family">';
			echo '<small>'.esc_attr__( 'Backup Font Family', 'labtechco' ).'</small><br>';
	  ...
	  echo '<div class="clear clr"></div> </div> <!-- .cs-ts-typography-row1 -->';
	  echo '<div class="cs-ts-typography-row2">';
      ...
      if( ! empty( $is_variant ) ) {
		...
		echo '<div class="ts-option-col-6 cs-ts-typography-dropdown-w cs-ts-typography-weight-w">';
        echo '<label class="cs-typography-variant">';
		echo '<small>'.esc_attr__( 'Font Variants', 'labtechco' ).'</small><br>';
		...
		if( ! empty( $is_text_transform ) ) {
			$variants = ( $is_google ) ? $googlefonts[$family_value] : $default_variants;
			$variants = ( $value['font'] === 'google' || $value['font'] === 'websafe' ) ? $variants : array( 'regular' );
			echo '<div class="ts-option-col-6 cs-ts-typography-dropdown-w cs-ts-typography-transform-w">';
			echo '<label class="cs-typography-text-transform">';
			echo '<small>'.esc_attr__( 'Text Transform', 'labtechco' ).'</small><br>';
	   ...
	   echo '<div class="clear clr"></div>  </div> <!-- .cs-ts-typography-row2 -->';
		...
		if( ! empty( $is_font_size ) ) {
			echo '<div class="ts-fancy-input-w">';
			echo '<label class="cs-typography-font-Size">';
			echo '<small>'.esc_attr__( 'Font Size', 'labtechco' ).'</small><br>';
			echo '<div class="input-append">
			<input type="number" class="cs-typo-font-size mini" min="1" max="200" step="1" title="'.esc_attr__( 'Font Size', 'labtechco' ).'" name="'. $this->element_name( '[font-size]' ) .'" value="'.$font_size_value.'">
		...
		if( ! empty( $is_line_height ) ) {
			...
			echo '<small>'.esc_attr__( 'Line Height', 'labtechco' ).'</small><br>';
			echo '<div class="input-append">
			<input type="number" class="cs-typo-line-height mini" min="1" max="200" step="1" title="'.esc_attr__( 'Line Height', 'labtechco' ).'" name="'. esc_attr($this->element_name( '[line-height]' )) .'" value="'.esc_attr($line_height_value).'">
			<span class="add-on">px</span>
			<div class="clear clr"></div>
			</div><div class="clear clr"></div>';
			echo '</label>';
		...
		if( ! empty( $is_letter_spacing ) ) {
			echo '<div class="ts-fancy-input-w">';
			echo '<label class="cs-typography-line-height">';
			echo '<small>'.esc_attr__( 'Letter Spacing', 'labtechco' ).'</small><br>';
			echo '<div class="input-append">
			<input type="number" class="cs-typo-line-height mini" min="0" max="20" step="0.1" title="'.esc_attr__( 'Letter Spacing', 'labtechco' ).'" name="'. esc_attr($this->element_name( '[letter-spacing]' )) .'" value="'.esc_attr($letter_spacing_value).'">
			<span class="add-on">px</span>
			<div class="clear clr"></div>
			</div><div class="clear clr"></div>';
			echo '</label>';
		 ...
		 */
		if( ! empty( $is_color ) ) {
			echo '<div class="ts-typography-font-color-w">';
			echo '<small>'.esc_attr__( 'Font Color', 'labtechco' ).'</small><br>';
			echo '<label class="ts-typography-font-color">';
      ...
      if( ! empty( $is_all_varients ) ) {
        ...
        $variants = ( $is_google ) ? $googlefonts[$family_value] : $default_variants;
        $variants = ( $value['font'] === 'google' || $value['font'] === 'websafe' ) ? $variants : array( 'regular' );
		echo '<div class="ts-typography-load-all-varient-w">';
        echo '<label class="cs-typography-font-Size">';
		echo '<div class="ts-typo-all-varient-input"><input type="checkbox" name="'. esc_attr($this->element_name( '[all-varients]' )) .'" value"on" '. checked( 'on', $all_varients_value, false ) .' /></div>';
		echo '<div class="ts-typo-load-all-varient-text">'. esc_attr__('Load all variations of the selected font (Like bold, italic etc.)','labtechco') .'<br><small>'. esc_attr__('Loading all font variations can slow down (a little) your webpage.','labtechco').'</small></div>';
		echo '<div class="clear clr"></div>';
        echo '</label>';
		echo '</div>';
      }
      ...
      echo esc_attr__( 'Error! Can not load json file.', 'labtechco' );
...
function ts_cs_get_google_fonts() {

	global $ts_cs_google_fonts;

	if( ! empty( $ts_cs_google_fonts ) ) {

		return $ts_cs_google_fonts;

	} else {

		ob_start();
		cs_locate_template( 'fields/themestek_typography/google-fonts.json' );
		$json = ob_get_clean();

		$ts_cs_google_fonts = json_decode( $json );
