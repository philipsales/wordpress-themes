...
if( is_page() ){
	...
	if( function_exists('tribe_is_upcoming') ){
		if (get_post_type()=='tribe_events'){
			$events_sidebar = !empty($labtechco_theme_options['sidebar_events']) ? esc_attr($labtechco_theme_options['sidebar_events']) : 'no' ; // Global settings
	...
	if( function_exists('tribe_is_upcoming') ){
		if ( get_post_type() == 'tribe_events' || tribe_is_upcoming() || tribe_is_month() || tribe_is_by_date() || tribe_is_day() || is_single('tribe_events')){
			$events_sidebar = ( isset($labtechco_theme_options['sidebar_events']) && trim($labtechco_theme_options['sidebar_events'])!='' ) ? esc_attr($labtechco_theme_options['sidebar_events']) : 'no' ; // Global settings
			if( $events_sidebar=='right' ){
				$sidebar_right = 'sidebar-events';
			}
		}
	...
	$bbpressSidebar = isset($labtechco_theme_options['sidebar_bbpress']) ? esc_attr($labtechco_theme_options['sidebar_bbpress']) : 'right' ;
