...
get_header(); ?>
	...
	<section id="primary" class="content-area <?php echo themestek_sanitize_html_classes(themestek_sidebar_class('content-area')); ?>">
		...
		<?php if ( have_posts() ) : ?>
			...
			$view   = ( !empty($labtechco_theme_options['services_cat_view']) ) ? $labtechco_theme_options['services_cat_view'] : 'overlay' ;
			$column = ( !empty($labtechco_theme_options['services_cat_column']) ) ? $labtechco_theme_options['services_cat_column'] : 'three' ;
			...
			while ( have_posts() ) : the_post();
				...
				<?php echo themestek_column_div('start', $column ); ?>
					<?php echo get_template_part( 'theme-parts/servicebox/servicebox', $view ); ?>
